...
# Summary function builds the output for gmnl. This contained all relevant values that the model outputs,
# as well as error terms and other important information that allows you to evaluate your model
#' @rdname gmnl
#' @method summary gmnl
#' @import stats
#' @export
...
# Print out everything that the summary function has structured and all other relevant outputs
# This is where to include the print out of any additional error functions or information desired
# cat() is our desired print function
#' @rdname gmnl
...
print.summary.gmnl <- function(x, digits = max(3, getOption("digits") - 2),
  ...
  cat(paste("\nMcFadden R^2: ", mcfadden.gmnl(x)))
  cat(paste("\nMcFadden R^2 Adjusted: ", mcfadden_adj.gmnl(x)))

  cat(paste("\nrho^2: ", rho2.gmnl(x)))
  cat(paste("\nrho^2 Adjusted: ", rho2_adj.gmnl(x)))

  cat(paste("\nNumber of observations:",  x$logLik$nobs))
  cat(paste("\nNumber of iterations:" , x$logLik$iterations))
  cat(paste("\nExit of MLE:", x$logLik$message))

  #If model is not MNL or LC, we output how many draws were done
...
#' @param object a gmnl object
#' @param returnObj optional bool, TRUE to return gmnl object with Rho^2 appended
#' FALSE to just return Rho^2 value
#'
#' @return gmnl object or Rho^2 value
#' @export
rho2.gmnl <- function(object, returnObj = FALSE){
  #Check to ensure correct object and data are given
  if (!inherits(object, "gmnl")) stop("Data must be of class gmnl for evaluation")
  if (!inherits(object$data, "dfidx")) stop("Data must be of class dfidx for Rho^2 evaluation")

  y <- gsub( " .*$", "", toString(object$call[2])) # We extract the name of the choice variable
  x <- 1
  fm <- as.formula(rlang::expr(!!rlang::sym(y) ~ !!x)) # Build the formula for our choice ~ 1 model
  ll_1 <- gmnl(fm, data = object$data)
  ll_1 <- ll_1$logLik$maximum[1] # Extract the log likelihood

  if(returnObj == FALSE){
    return(round(c(1 - object$logLik$maximum[1] / ll_1),7))
  }else{
    object$rho2 <- round(c(1 - object$logLik$maximum[1] / ll_1),7)
    return(object)
  }
}

#' Adjusted Rho^2 Value
#'
#' @param object gmnl object of a computed model
#' @param returnObj optional bool, TRUE to return gmnl object with adj Rho^2 appended
#' FALSE to just return Rho^2 value
#'
#' @return gmnl object or adjusted Rho^2 value
#' @export
rho2_adj.gmnl <- function(object, returnObj = FALSE){
  #Check to ensure correct object and data are given
  if (!inherits(object, "gmnl")) stop("Data must be of class gmnl for evaluation")
  if (!inherits(object$data, "dfidx")) stop("Data must be of class dfidx for Rho^2 evaluation")
  ...
  y <- gsub( " .*$", "", toString(object$call[2])) # We extract the name of the choice variable
  x <- 1
  fm <- as.formula(rlang::expr(!!rlang::sym(y) ~ !!x)) # Build the formula for our choice ~ 1 model -> REFER TO THIS AS rlang::expr
  ll_1 <- gmnl(fm, data = object$data)
  ll_1 <- ll_1$logLik$maximum[1] # Extract the log likelihood

  K <- length(object$coefficients)  # Number of estimated parameters in model (total)

  if(returnObj == FALSE){
    return(round(c(1 - (object$logLik$maximum[1]  - K)/ ll_1),7))
  }else{
    object$rho2adj <- round(c(1 - ((object$logLik$maximum[1] - K) / ll_1)) ,7)
    return(object)
  }
}
...
#' McFadden R^2 Value
#'
#' @param object gmnl object
#' @param returnObj optional TRUE/FALSE for whether a gmnl object is returned containing the mcfadden value
#'
#' @return mcfadden value OR gmnl object containing mcfadden value
#' @export
#'
mcfadden.gmnl <- function(object, returnObj = FALSE){
  #Check to ensure correct object and data are given
  if (!inherits(object, "gmnl")) stop("Data must be of class gmnl for evaluation")
  if (!inherits(object$data, "dfidx")) stop("Data must be of class dfidx for McFadden R^2 evaluation")

  ll_0 <- sum_log_inv_num_alternatives(object) # Extract the log likelihood

  if(returnObj == FALSE){
    return(round(c(1 - object$logLik$maximum[1] / ll_0),7))
  }else{
    object$mcfadden <- round(c(1 - object$logLik$maximum[1] / ll_0),7)
    return(object)
  }
}

#' McFadden R^2 Adjusted
#'
#' @param object gmnl object
#' @param returnObj optional TRUE/FALSE for whether a gmnl object is returned containing the mcfadden value
#'
...
#' @export
#'
mcfadden_adj.gmnl <- function(object, returnObj = FALSE){
  #Check to ensure correct object and data are given
  if (!inherits(object, "gmnl")) stop("Data must be of class gmnl for evaluation")
  if (!inherits(object$data, "dfidx")) stop("Data must be of class dfidx for McFadden R^2 evaluation")
  ...
  if(returnObj == FALSE){
    return(round(c(1 - (object$logLik$maximum[1]  - K)/ ll_0),7))
  }else{
    object$mcfaddenadj <- round(c(1 - (object$logLik$maximum[1] - K) / ll_0),7)
    return(object)
  }
}
...
#'
#' A generic function to collect coefficients and summary statistics from a \code{gmnl} object.
#'
#' @param object a \code{gmnl} object,
#' @param alpha level of the confidence intervals,
#' @param ... further arguments,
#' @details For more details see package \pkg{memisc}
#' @export
getSummary.gmnl <- function(object, alpha = 0.05, ... ){
  smry <- summary(object)
  coef <- smry$CoefTable
  ...
  N <-  object$logLik$nobs
  ll <- logLik(object)
  mcfadden <- mcfadden.gmnl(object)
  sumstat <- c(logLik = ll, deviance = NA, AIC = AIC(object), BIC = BIC(object), N = N,
               LR = NA, df = NA, p = NA, Aldrich.Nelson = NA, McFadden = mcfadden, Cox.Snell = NA, #McFadden on this line should be updated
               Nagelkerke = NA)
  list(coef = coef, sumstat = sumstat, contrasts = object$contrasts,
...
cor.gmnl <- function(x, Q = NULL){
  ...
  nr  <- names(x$ranp)
  D   <- diag(sqrt(diag(V)))
  Rho <- solve(D) %*% V %*% solve(D)  # matrix multiplication here
